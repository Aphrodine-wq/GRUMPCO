...
async function configureAIProviders(mode) {
  ...
  log(`  ${colors.cyan}3. Ollama${colors.reset} (Local)`);
  log(`     Run models locally | No API key needed`);
  log(`     ${colors.dim}https://ollama.com/${colors.reset}\n`);
  ...
  if (addNow) {
    const provider = await prompt('Which provider? (nvidia/openrouter/skip)', 'skip');
