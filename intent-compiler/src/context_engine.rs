...
use crate::semantics::{ImplicitRequirement, Contradiction, StyleSuggestion};
...
pub fn analyze_context(
    ...
    let moat_strength = if style_suggestions.len() > 2 { 0.7 } else { 0.4 };
    ...
    let overall_score = (
        market_analysis.viability_score * market_weight +
    ...
    let should_build = matches!(verdict.decision, VerdictType::BuildNow | VerdictType::BuildButPivot);
    ...
    let recommendation = match verdict.decision {
        ...
        VerdictType::BuildButPivot => format!(
            "Your core idea is good, but you're targeting the wrong market.
             Build this instead: {}",
            verdict.if_you_pivot_try_instead.first().cloned().unwrap_or_default()
        ),
        VerdictType::Skip => "Market is too saturated. Your effort is better spent elsewhere.".to_string(),
    ...
    ContextAnalysis {
        ...
        market: MarketInsights {
            ...
            competition: format!("{:?}", market_analysis.primary_opportunity.competition_level),
            market_viability: market_analysis.viability_score,
            addressable_segments: market_analysis.alternative_opportunities
                .iter()
                .map(|s| s.name.clone())
                .collect(),
            realistic_revenue_year_3: market_analysis.revenue_model.year_3_revenue_projection.to_string(),
            exit_potential: market_analysis.primary_opportunity.exit_potential.clone(),
            key_risks: market_analysis.risk_factors
                .iter()
                .take(3)
        ...
        product: ProductInsights {
            ...
            moat_strength,
            feature_overlap_with_competitors: assess_overlap(&features, &market_analysis.competitive_landscape),
        ...
        execution: ExecutionInsights {
            go_to_market_difficulty: market_analysis.go_to_market.initial_marketing_budget as f32 / 100000.0,
            ...
            months_to_profitability: market_analysis.primary_opportunity.time_to_profitability_months,
...
fn calculate_technical_viability(
    ...
    let contradiction_penalty = contradictions.iter().map(|c| c.severity * 15.0).sum::<f32>();
    (base_score - contradiction_penalty).max(20.0)
}

fn calculate_product_viability(
    features: &[String],
    contradictions: &[Contradiction],
) -> f32 {
    let feature_clarity = if features.len() > 15 { 30.0 } else { 80.0 };
    let contradiction_impact = contradictions.iter().map(|c| c.severity * 20.0).sum::<f32>();
...
fn calculate_execution_difficulty(features: &[String]) -> f32 {
    ...
    let integration_complexity = if features.iter().any(|f| f.contains("payment") || f.contains("external")) {
        30.0
    } else {
        10.0
    };
...
fn make_verdict(
    ...
    FinalVerdict {
        decision,
        rationale: "Synthesized from market, technical, product, and execution analysis".to_string(),
        best_case_scenario: format!(
            ...
            (overall_score * 0.5) as i32, (overall_score * 2.0) as i32
        ),
        ...
        if_you_pivot_try_instead: market_analysis.alternative_opportunities
            .iter()
...
fn infer_stack(features: &[String]) -> Vec<String> {
    let mut stack = vec![];
    if features.iter().any(|f| f.contains("user") || f.contains("profile")) {
    ...
    if features.iter().any(|f| f.contains("realtime") || f.contains("notification")) {
...
fn describe_ideal_customer(features: &[String]) -> String {
    if features.iter().any(|f| f.contains("business") || f.contains("enterprise")) {
        "Mid-market companies, $10M-$100M revenue".to_string()
    } else if features.iter().any(|f| f.contains("creator") || f.contains("freelance")) {
...
fn assess_overlap(_features: &[String], competitors: &[crate::market_engine::Competitor]) -> String {
    if competitors.is_empty() {
        "No direct competitors identified".to_string()
    } else {
        format!("High overlap with {}", 
            competitors.first().map(|c| &c.name).unwrap_or(&"existing solutions".to_string())
        )
    }
}
...
fn estimate_retention_risk(features: &[String]) -> f32 {
    if features.iter().any(|f| f.contains("habit") || f.contains("wellness")) {
        0.8 // High churn risk
...
fn infer_team_requirements(features: &[String], complexity: f32) -> Vec<String> {
    ...
    if features.iter().any(|f| f.contains("payment") || f.contains("financial")) {
...
fn identify_failure_modes(features: &[String], risks: &[crate::market_engine::RiskFactor]) -> Vec<String> {
    let mut modes = vec![];
