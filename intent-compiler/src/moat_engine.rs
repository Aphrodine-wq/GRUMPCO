...
pub fn calculate_moat_strength(
    ...
    MoatSystem {
        founder_dna_layer: FounderDNAMoat {
            ...
            reason: format!("Analyzed {} founders. Competitors don't have this data.", founders_analyzed),
        },
        ...
        market_intel_layer: MarketIntelMoat {
            ...
            reason: format!("Real-time feeds from {} sources. Static analysis can't compete.", market_feeds),
        },
        ...
        network_layer: NetworkMoat {
            ...
            reason: format!("{} founders = unstoppable network effects. Stronger every day.", network_size),
        },
        
        accuracy_layer: AccuracyMoat {
            verdicts_tracked: (founders_analyzed / 10).max(1),
            accuracy_percentage: accuracy,
            public_track_record: format!("{}% accuracy (tracked over time)", (accuracy * 100.0) as u32),
...
pub fn moat_revenue_model() -> MoatRevenue {
    MoatRevenue {
        ...
        pro_tier: "$299/month: Unlimited verdicts + founder DNA profiling + early access to pivots".to_string(),
        enterprise_tier: "$5K-$50K/month: API access, white-label, custom integrations for VCs".to_string(),
        data_licensing: "Sell anonymized market patterns to investors: '$10K-$100K per insight'".to_string(),
        year_1_revenue: "$500K (early founders + VCs)".to_string(),
        year_3_revenue: "$10M+ (10K+ paying founders + VC partnerships)".to_string(),
        year_5_revenue: "$50M+ (industry standard, data licensing, acquisition offers from Stripe/Y Combinator)".to_string(),
    }
}
