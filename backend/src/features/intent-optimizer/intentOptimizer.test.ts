  ...
  describe('result structure validation', () => {
    ...
    it('should have valid constraint structure', async () => {
      ...
      if (result.constraints.length > 0) {
        ...
        expect(['technical', 'business', 'regulatory', 'resource']).toContain(constraint.type);
        expect(['must', 'should', 'nice_to_have']).toContain(constraint.priority);
      }
    });
...
describe('Intent Optimizer Routes', () => {
  ...
  describe('POST /api/intent/optimize', () => {
    ...
    it('should validate mode field', async () => {
      const body = { intent: 'test', mode: 'invalid' } as { intent: string; mode: string };
    ...
    it('should reject empty intent', async () => {
      const body = { intent: '   ', mode: 'codegen' } as { intent: string; mode: string };
    ...
    it('should accept valid request', async () => {
      ...
      if (!body.intent || typeof body.intent !== 'string') {
        hasError = true;
      } else if (!body.mode || !['codegen', 'architecture'].includes(body.mode)) {
        hasError = true;
