...
class DatabaseService {
   ...
   * Public so that extracted query modules can use the statement cache.
   */
  getStatement(sql: string): DatabaseType.Statement {
  ...
  // Domain-specific query methods are registered from external modules below.
  // See: integrationQueries.ts, platformQueries.ts, messagingQueries.ts

  // These declarations allow TypeScript to see the mixin methods on the class.
  // The actual implementations are registered at the bottom of this file.
  declare saveAuditLog: (record: AuditLogRecord) => Promise<void>;
  declare getAuditLogs: (options?: {
    userId?: string;
    category?: string;
    limit?: number;
    offset?: number;
  }) => Promise<AuditLogRecord[]>;
  declare saveIntegration: (record: IntegrationRecord) => Promise<void>;
  declare getIntegration: (id: string) => Promise<IntegrationRecord | null>;
  declare getIntegrationsForUser: (userId: string) => Promise<IntegrationRecord[]>;
  declare getIntegrationByProvider: (
    userId: string,
    provider: IntegrationProviderId
  ) => Promise<IntegrationRecord | null>;
  declare deleteIntegration: (id: string) => Promise<void>;
  declare saveOAuthToken: (record: OAuthTokenRecord) => Promise<void>;
  declare getOAuthToken: (
    userId: string,
    provider: IntegrationProviderId
  ) => Promise<OAuthTokenRecord | null>;
  declare deleteOAuthToken: (userId: string, provider: IntegrationProviderId) => Promise<void>;
  declare saveIntegrationSecret: (record: IntegrationSecretRecord) => Promise<void>;
  declare getIntegrationSecret: (
    userId: string,
    provider: IntegrationProviderId,
    name: string
  ) => Promise<IntegrationSecretRecord | null>;
  declare deleteIntegrationSecret: (
    userId: string,
    provider: IntegrationProviderId,
    name: string
  ) => Promise<void>;
  declare saveHeartbeat: (record: HeartbeatRecord) => Promise<void>;
  declare getHeartbeat: (id: string) => Promise<HeartbeatRecord | null>;
  declare getEnabledHeartbeats: () => Promise<HeartbeatRecord[]>;
  declare getHeartbeatsForUser: (userId: string) => Promise<HeartbeatRecord[]>;
  declare deleteHeartbeat: (id: string) => Promise<void>;
  declare saveApprovalRequest: (record: ApprovalRequestRecord) => Promise<void>;
  declare getApprovalRequest: (id: string) => Promise<ApprovalRequestRecord | null>;
  declare getPendingApprovals: (userId: string) => Promise<ApprovalRequestRecord[]>;
  declare saveSwarmAgent: (record: SwarmAgentRecord) => Promise<void>;
  declare getSwarmAgent: (id: string) => Promise<SwarmAgentRecord | null>;
  declare getSwarmChildren: (parentId: string) => Promise<SwarmAgentRecord[]>;
  declare getRunningSwarmAgents: () => Promise<SwarmAgentRecord[]>;
  declare saveSkill: (record: SkillRecord) => Promise<void>;
  declare getSkill: (id: string) => Promise<SkillRecord | null>;
  declare getSkillByName: (name: string) => Promise<SkillRecord | null>;
  declare getActiveSkills: () => Promise<SkillRecord[]>;
  declare saveMemoryRecord: (record: MemoryRecord) => Promise<void>;
  declare getMemoryRecord: (id: string) => Promise<MemoryRecord | null>;
  declare getMemoryRecordsByType: (
    userId: string,
    type: string,
    limit?: number
  ) => Promise<MemoryRecord[]>;
  declare getRecentMemories: (userId: string, limit?: number) => Promise<MemoryRecord[]>;
  declare deleteMemoryRecord: (id: string) => Promise<void>;
  declare saveCostBudget: (record: CostBudgetRecord) => Promise<void>;
  declare getCurrentBudget: (userId: string) => Promise<CostBudgetRecord | null>;
  declare saveRateLimit: (record: RateLimitRecord) => Promise<void>;
  declare getRateLimit: (userId: string, resource: string) => Promise<RateLimitRecord | null>;
  declare incrementRateLimit: (
    userId: string,
    resource: string
  ) => Promise<{ allowed: boolean; remaining: number }>;
  declare saveBrowserAllowlist: (record: BrowserAllowlistRecord) => Promise<void>;
  declare getBrowserAllowlist: (userId: string) => Promise<BrowserAllowlistRecord[]>;
  declare isDomainAllowed: (
    userId: string,
    domain: string
  ) => Promise<BrowserAllowlistRecord | null>;
  declare deleteBrowserAllowlist: (userId: string, domain: string) => Promise<void>;
  declare saveSlackToken: (record: SlackTokenRecord) => Promise<void>;
  declare getSlackToken: (userId: string, workspaceId: string) => Promise<SlackTokenRecord | null>;
  declare getSlackTokenByWorkspace: (workspaceId: string) => Promise<SlackTokenRecord | null>;
  declare getSlackTokensForUser: (userId: string) => Promise<SlackTokenRecord[]>;
  declare deleteSlackToken: (userId: string, workspaceId: string) => Promise<void>;
  declare getConversationMemory: (
    platform: string,
    platformUserId: string
  ) => Promise<ConversationMemoryRecord | null>;
  declare saveConversationMemory: (record: ConversationMemoryRecord) => Promise<void>;
  declare getMessagingSubscriptions: (userId: string) => Promise<MessagingSubscriptionRecord[]>;
  declare saveMessagingSubscription: (record: MessagingSubscriptionRecord) => Promise<void>;
  declare deleteMessagingSubscription: (
    userId: string,
    platform: string,
    platformUserId: string
  ) => Promise<void>;
  declare saveSlackUserPairing: (record: SlackUserPairingRecord) => Promise<void>;
  declare getGrumpUserIdFromSlack: (
    slackUserId: string,
    workspaceId: string
  ) => Promise<string | null>;
  declare saveReminder: (record: ReminderRecord) => Promise<void>;
  declare getDueReminders: (before: string) => Promise<ReminderRecord[]>;
  declare getRemindersForUser: (userId: string) => Promise<ReminderRecord[]>;
  declare setReminderNotified: (id: string) => Promise<void>;
}

// Register domain-specific query methods onto DatabaseService prototype
import { registerIntegrationQueries } from './integrationQueries.js';
import { registerPlatformQueries } from './platformQueries.js';
import { registerMessagingQueries } from './messagingQueries.js';

registerIntegrationQueries(DatabaseService.prototype);
registerPlatformQueries(DatabaseService.prototype);
registerMessagingQueries(DatabaseService.prototype);
