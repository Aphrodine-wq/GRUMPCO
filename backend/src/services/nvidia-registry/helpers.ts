...
export function getNvidiaModelsByCapability(
  cap: NvidiaModelCapability,
): NvidiaModelConfig[] {
  return NVIDIA_MODEL_REGISTRY.filter((m) => m.capabilities.includes(cap));
}

export function getNvidiaModelsByPublisher(
  publisher: string,
): NvidiaModelConfig[] {
  return NVIDIA_MODEL_REGISTRY.filter(
    (m) => m.publisher.toLowerCase() === publisher.toLowerCase(),
  );
}

export function getNvidiaChatModels(): NvidiaModelConfig[] {
  return NVIDIA_MODEL_REGISTRY.filter(
    (m) => m.capabilities.includes("chat") && m.supportsTools,
  );
}
...
export function getNvidiaVisionModels(): NvidiaModelConfig[] {
  return NVIDIA_MODEL_REGISTRY.filter(
    (m) => m.capabilities.includes("vision") || m.multimodal,
  );
}

export function getNvidiaEmbeddingModels(): NvidiaModelConfig[] {
  return NVIDIA_MODEL_REGISTRY.filter((m) =>
...
export function getCheapestNvidiaModel(): NvidiaModelConfig {
  return NVIDIA_MODEL_REGISTRY.reduce((cheapest, model) => {
    ...
    const cheapestCost =
      cheapest.costPerTokenInput + cheapest.costPerTokenOutput;
