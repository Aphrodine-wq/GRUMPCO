...
export class IntentDrivenTemplate {
  ...
  generateFromIntent(
    ...
    const frontend = this.selectPrimary(hints.frontend, [
      "react",
      "vue",
      "svelte",
      "nextjs",
    ]);
    const backend = this.selectPrimary(hints.backend, [
      "express",
      "fastapi",
      "nestjs",
    ]);
    const database = this.selectPrimary(hints.database, [
      "postgres",
      "mongodb",
      "sqlite",
    ]);
    const language = this.selectPrimary(hints.language, [
      "typescript",
      "javascript",
      "python",
    ]);
    ...
    if (language === "typescript" || language === "javascript") {
      ...
      files.push(
        this.generatePackageJson(safeName, frontend, backend, database),
      );
  ...
  private selectPrimary(
    ...
    const match = hints.find((h) =>
      defaults.some((d) => h.toLowerCase().includes(d)),
    );

    return match
      ? defaults.find((d) => match.toLowerCase().includes(d))!
      : defaults[0];
  }
