...
export async function submitAnswer(
  ...
  if (session.status !== "collecting") {
    throw new Error(
      `Cannot submit answers to session ${sessionId} in status ${session.status}`,
    );
  }

  const question = session.questions.find(
    (q) => q.id === answerRequest.questionId,
  );
  if (!question) {
  ...
  await db.saveSpec(session);
  logger.info(
    { sessionId, questionId: answerRequest.questionId },
    "Answer submitted",
  );
    ...
    case "multi-choice":
      ...
      for (const v of value) {
        if (!question.options?.includes(v)) {
          throw new Error(
            `All selections must be from: ${question.options?.join(", ")}`,
          );
        }
      }
      break;
    ...
    case "text":
      ...
      if (question.validation) {
        if (
          ...
          throw new Error(
            `Text must be at least ${question.validation.minLength} characters`,
          );
        }
        if (
          ...
          throw new Error(
            `Text must be at most ${question.validation.maxLength} characters`,
          );
        }
...
export async function generateSpecification(
  ...
  const hasCDD =
    useDesignContext &&
    (request.designContext as SpecDesignContext)?.creativeDesignDoc != null;
...
Be thorough and specific. ${useDesignContext ? "Use all provided design context (project description, PRD overview, Creative Design Document)." : "Include all relevant details from the answers."}`;
  ...
  if (useDesignContext && request.designContext) {
    ...
    if (ctx.prdOverview) {
      if (ctx.prdOverview.vision)
        userPrompt += `\n\nPRD vision: ${ctx.prdOverview.vision}`;
      if (ctx.prdOverview.problem)
        userPrompt += `\nPRD problem: ${ctx.prdOverview.problem}`;
      if (ctx.prdOverview.solution)
