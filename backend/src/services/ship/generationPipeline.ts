...
export interface PipelineContext {
  ...
  /** Architecture drift report (populated during validation) */
  driftReport?: import('../../types/architecture.js').DriftReport;
  /** Metadata */
  metadata: {
    startTime: number;
    techStack?: string;
...
export class GenerationPipeline {
  ...
  private async validate(context: PipelineContext): Promise<void> {
    ...
    // ── Drift Detection ──────────────────────────────────────────────────
    // If we have an architecture, check for architectural drift
    try {
      if (context.architecture) {
        const { generateArchitecture } = await import('./architectureService.js');
        const { analyzeDrift, formatDriftReport } = await import('./driftDetector.js');

        // Re-derive the full architecture object for ruleset building
        const archResponse = await generateArchitecture({
          projectDescription: context.request,
          techStack: context.metadata.techStack ? [context.metadata.techStack] : undefined,
        });

        if (archResponse.architecture) {
          const report = analyzeDrift(archResponse.architecture, files);
          context.driftReport = report;
          ...
          if (!report.passes) {
            logger.warn(
              ...
              `Drift detection found ${report.summary.errors} error(s)`
            );
          } else if (report.summary.total > 0) {
            logger.info(
              { warnings: report.summary.warnings, info: report.summary.info },
              'Drift detection passed with advisories'
            );
          }
        }
      }
    } catch (driftErr) {
      // Drift detection should never block the pipeline
      logger.warn({ error: (driftErr as Error).message }, 'Drift detection skipped due to error');
    }
  }
