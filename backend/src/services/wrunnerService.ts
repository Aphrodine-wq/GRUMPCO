      ...
      try {
        ...
        if (sampleFiles.length > 0) {
          const [analyses, securityScans, perfScans] = await Promise.all([
            Promise.allSettled(sampleFiles.map(f => analyzeCode(f.content, f.language || 'typescript'))),
            Promise.allSettled(sampleFiles.map(f => scanSecurity(f.content, f.language || 'typescript'))),
            Promise.allSettled(sampleFiles.map(f => optimizePerformance(f.content, f.language || 'typescript'))),
          ]);
          ...
          if (analysisResults.length > 0 || securityResults.length > 0 || perfResults.length > 0) {
            ...
            if (analysisResults.length > 0) {
              ...
              const criticalSmells = allSmells.filter(s => s.severity === 'critical' || s.severity === 'high');
              claudeCodeInsights += `- Code Quality: ${allSmells.length} code smells detected (${criticalSmells.length} critical/high)\n`;
            }
            if (securityResults.length > 0) {
              const criticalSecurity = securityResults.filter(s => s.severity === 'critical' || s.severity === 'high');
        ...
        log.warn({ error: (error as Error).message }, 'Claude Code analysis in WRunner failed, continuing without it');
      }
    }
...
Generate a comprehensive analysis with actionable fixes.`;
    ...
    log.error({ sessionId: session.sessionId, error: (error as Error).message }, 'WRunner analysis failed');
...
export function generateFixPlan(analysis: WRunnerAnalysis): {
  ...
  for (const issue of analysis.issues) {
    ...
    const isAutoFixable =
      (issue.category === 'missing' || issue.category === 'quality' || issue.category === 'inconsistency') &&
...
export function hasAutoFixableIssues(analysis: WRunnerAnalysis): boolean {
  return analysis.autoFixable && analysis.issues.some((issue) => {
    const isAutoFixable =
      (issue.category === 'missing' || issue.category === 'quality' || issue.category === 'inconsistency') &&
      issue.severity !== 'critical' &&
      issue.suggestedFixes.length > 0;
