...
function getDefaultModel(): string {
  return process.env.NIM_EMBED_MODEL || process.env.RAG_EMBED_MODEL || DEFAULT_EMBED_MODEL;
}
...
export async function embed(texts: string[], options?: EmbedOptions): Promise<number[][]> {
  ...
  if (!res.ok) {
    ...
    logger.warn({ status: res.status, body: t.slice(0, 500) }, 'NIM embeddings error');
  ...
  const embeddings = byIndex
    ? raw.sort((a, b) => (a.index ?? 0) - (b.index ?? 0)).map((d) => d.embedding)
