/**
 * G-Agent Tool Handlers
 * Implements tool execution for expanded G-Agent capabilities:
...
export async function dbMigrateDryrun(migrationSql: string, ctx: ToolContext): Promise<ToolResult> {
  try {
    await writeAuditLog({
      userId: ctx.userId,
      action: 'gagent.db_migrate_dryrun',
      category: 'tool',
      target: 'database',
...
export async function httpGet(
  ...
  if (!isUrlAllowed(url, ctx.allowlist)) {
    return {
      success: false,
      error: `URL not in allowlist. Add the domain to your G-Agent allowlist first.`,
    };
  }
...
export async function pipelineTrigger(
  ...
  try {
    await writeAuditLog({
      userId: ctx.userId,
      action: 'gagent.pipeline_trigger',
      category: 'tool',
...
/**
 * Execute a G-Agent tool by name
 */
export async function executeGAgentTool(
  toolName: ToolName,
