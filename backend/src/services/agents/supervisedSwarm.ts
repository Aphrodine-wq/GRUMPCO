...
import type { CompressedContext as _CompressedContext } from '../rag/contextCompressor.js';
...
export class SupervisedSwarmOrchestrator extends EventEmitter {
  ...
  initialize(agentsPerRole: number = 1): void {
    ...
    for (const role of roles) {
      for (let i = 0; i < Math.min(agentsPerRole, this.config.maxAgentsPerRole); i++) {
  ...
  private async kimiCreatePlan(query: string, _context?: string): Promise<TaskPlan> {
    ...
    subtasks.push({
      ...
      priority: 'high',
      dependsOn: subtasks.slice(0, -1).map((_, i) => `${subtasks[i].role}_${i}`),
    });

    // Determine complexity
    let complexity: TaskPlan['estimatedComplexity'] = 'simple';
    if (subtasks.length > 4) complexity = 'moderate';
    if (subtasks.length > 6 || queryLower.includes('complex')) complexity = 'complex';
  ...
  private async kimiReview(subtask: AgentSubtask): Promise<SupervisorReview> {
    ...
    if (score >= this.config.autoApproveThreshold) {
      ...
      decision = 'approve';
    } else if (score >= threshold - 0.15 && subtask.attempts < subtask.maxAttempts) {
      decision = 'revise';
    } else {
      decision = 'reject';
    }
  ...
  private async kimiSynthesize(task: SupervisedTask): Promise<string> {
    ...
    parts.push('');
    parts.push('## Summary');
    parts.push(`Completed analysis with ${approvedSubtasks.length} specialist agents.`);
    parts.push('');
...
export class SupervisedSwarmService {
   ...
   */
  getSwarm(name: string, config?: Partial<SupervisedSwarmConfig>): SupervisedSwarmOrchestrator {
