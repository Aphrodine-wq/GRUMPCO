...
class SupabaseClient {
  ...
  private async fetch<T>(
    ...
    const apiKey = useServiceRole && this.serviceRoleKey ? this.serviceRoleKey : this.anonKey;
    ...
    try {
      ...
      if (!res.ok) {
        ...
        logger.error({ status: res.status, error: errorText, endpoint }, 'Supabase API error');
        return null;
      }
      ...
      logger.error({ error: (err as Error).message, endpoint }, 'Supabase fetch error');
      return null;
    }
  }
  ...
  async rpc<T>(functionName: string, params: Record<string, unknown> = {}): Promise<T | null> {
  ...
  async uploadFile(
    ...
    try {
      ...
      if (!res.ok) {
        const errorText = await res.text();
        logger.error({ status: res.status, error: errorText }, 'Supabase storage upload error');
        return null;
      }
...
class FirebaseAdminClient {
  ...
  private async fetch<T>(url: string, options: RequestInit = {}): Promise<T | null> {
    try {
      ...
      if (!res.ok) {
        const errorText = await res.text();
        logger.error({ status: res.status, error: errorText, url }, 'Firebase API error');
        return null;
      }
      ...
      logger.error({ error: (err as Error).message, url }, 'Firebase fetch error');
      return null;
    }
  }
        ...
        compositeFilter?: {
          ...
          filters: Array<{
            fieldFilter: {
              ...
              op: string;
              value: { stringValue?: string; integerValue?: number; booleanValue?: boolean };
            };
          }>;
        };
      };
  ...
  async createDocument<T>(
    ...
    const result = await this.fetch<{ name: string; fields: Record<string, unknown> }>(url, {
      method: 'POST',
  ...
  async uploadFile(
    ...
    try {
      ...
      if (!res.ok) {
        const errorText = await res.text();
        logger.error({ status: res.status, error: errorText }, 'Firebase storage upload error');
        return null;
      }
...
// Firebase factory
export function createFirebaseClient(projectId: string, accessToken: string): FirebaseAdminClient {
  ...
  return [
    ...
    },
    {
      ...
      columns: [
        ...
        { name: 'theme', type: 'text', nullable: true, defaultValue: "'light'" },
        { name: 'notifications', type: 'boolean', nullable: false, defaultValue: 'true' },
  ...
  return [
    {
      ...
      columns: [
        ...
        { name: 'inventory_count', type: 'int', nullable: false, defaultValue: '0' },
    ...
    },
    {
      ...
      columns: [
        ...
        { name: 'shipping', type: 'decimal', nullable: false, defaultValue: '0' },
