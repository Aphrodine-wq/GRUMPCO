...
class WebSocketStreamingService {
  ...
  private handleMessage(session: StreamSession, data: Buffer): void {
    ...
    try {
      ...
      logger.error({ error: (error as Error).message }, 'Failed to parse WebSocket message');
    }
  }
  ...
  private async handleStartStream(
    ...
    logger.info({ sessionId: session.id, streamId }, 'Starting WebSocket stream');
    ...
    try {
      ...
      logger.error({ streamId, error: (error as Error).message }, 'Stream error');
   ...
   */
  private handleCancelStream(session: StreamSession, message: WebSocketMessage): void {
