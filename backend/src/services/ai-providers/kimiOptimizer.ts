...
export function shouldRouteToKimi(request: {
  ...
  if (request.requiresTools) {
    confidence -= 0.2;
    reasons.push(
      "Note: Tool use may benefit from Llama 405B for complex scenarios",
    );
  }
...
export function estimateKimiSavings(
  ...
  const kimiCost =
    ((inputTokens / 1_000_000) * 0.6 + (outputTokens / 1_000_000) * 0.6) *
    requestCount;

  const savings = llama405bCost - kimiCost;
  const savingsPercent =
    llama405bCost > 0 ? (savings / llama405bCost) * 100 : 0;
...
export function getKimiTaskConfig(
  taskType: "coding" | "chat" | "analysis" | "creative",
) {
  const configs = {
    coding: {
      temperature: 0.1,
...
export function getKimiRoutingDecision(input: KimiRoutingInput): {
  ...
  if (
    ...
    !["en", "english"].includes(input.detectedLanguage.toLowerCase())
  ) {
    kimiScore += 30;
    rationale.push(
      `Multilingual content (${input.detectedLanguage}) - Kimi excels here`,
    );
  }
  ...
  if (input.toolsRequested) {
    kimiScore -= 20;
    rationale.push(
      "Tool use required - Llama 405B may be more reliable for complex tool chains",
    );
  }
  ...
  const savings = estimateKimiSavings(
