...
import { getDatabase, databaseSupportsRawDb, type DatabaseService } from '../../db/database.js';
import logger from '../../middleware/logger.js';
import { MODEL_REGISTRY } from '@grump/ai-core';
import { getAlertingService } from '../infra/alerting.js';
...
export class CostAnalytics {
   ...
   * Record a cost event
   */
  public async recordCost(record: Omit<CostRecord, 'id' | 'timestamp'>): Promise<void> {
  ...
  public calculateCost(model: string, inputTokens: number, outputTokens: number): number {
    ...
    const inputCost = ((modelConfig.costPerMillionInput || 0) * inputTokens) / 1_000_000;
    const outputCost = ((modelConfig.costPerMillionOutput || 0) * outputTokens) / 1_000_000;
  ...
  public async checkBudget(userId: string): Promise<{
    ...
    // Check limits
    const dailyExceeded = budget.dailyLimitUsd && dailyUsed > budget.dailyLimitUsd;
    const monthlyExceeded = budget.monthlyLimitUsd && monthlyUsed > budget.monthlyLimitUsd;
  ...
  public async getRecommendations(userId: string): Promise<string[]> {
    ...
    const expensiveModels = ['claude-sonnet-4-20250514', 'claude-3-5-sonnet-20241022'];
    const expensiveCost = expensiveModels.reduce(
      (sum, model) => sum + (summary.costByModel[model] || 0),
      0
    );
    const expensivePercent = summary.totalCost > 0 ? (expensiveCost / summary.totalCost) * 100 : 0;
