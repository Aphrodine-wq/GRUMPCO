...
export class CostOptimizer {
  ...
  public analyzeComplexity(
    ...
    const requiresReasoning =
      /\b(analyze|evaluate|compare|explain why|reasoning|logic|deduce)\b/i.test(
        fullText,
      );

    const requiresCreativity =
      /\b(creative|innovative|unique|original|brainstorm|imagine)\b/i.test(
        fullText,
      );
    ...
    const hasCodeGeneration =
      /\b(code|function|class|implement|build|develop|program)\b/i.test(
        fullText,
      ) ||
      mode === "ship" ||
      mode === "codegen";
  ...
  public selectModel(
    ...
    const allowedModels = MODEL_REGISTRY.filter((model) => {
      ...
      if (
        ...
        !options.allowedProviders.includes(model.provider)
      ) {
        return false;
      }
    ...
    const sortedModels = allowedModels.sort((a, b) => {
      const costA =
        (a.costPerMillionInput || 0) + (a.costPerMillionOutput || 0);
      const costB =
        (b.costPerMillionInput || 0) + (b.costPerMillionOutput || 0);
      return costA - costB;
    });
    ...
    if (complexity.score < 30 && !complexity.factors.requiresAccuracy) {
      ...
      const midTierModels = sortedModels.filter((m) => {
        const cost =
          (m.costPerMillionInput || 0) + (m.costPerMillionOutput || 0);
        return cost > 1 && cost < 10;
      });

      selectedModel =
        midTierModels.length > 0 ? midTierModels[0] : sortedModels[0];
  ...
  private estimateCost(model: ModelConfig, messageLength: number): number {
    ...
    const inputCost =
      ((model.costPerMillionInput || 0) * estimatedTokens) / 1_000_000;

    // Assume output is 50% of input length
    const outputTokens = Math.ceil(estimatedTokens * 0.5);
    const outputCost =
      ((model.costPerMillionOutput || 0) * outputTokens) / 1_000_000;
    ...
    return {
      ...
      cheapModelPercentage:
        this.costSavings.totalRequests > 0
          ? (this.costSavings.cheapModelUsed / this.costSavings.totalRequests) *
            100
          : 0,
    };
  }
  ...
  public getRecommendations(
    ...
    const recommendations = MODEL_REGISTRY.map((model) => {
      ...
      if (complexity.score < 30) {
        ...
        const cost =
          (model.costPerMillionInput || 0) + (model.costPerMillionOutput || 0);
      ...
      if (
        complexity.factors.hasCodeGeneration &&
        model.capabilities.includes("code")
      ) {
        suitability += 20;
      }
      if (
        complexity.factors.requiresReasoning &&
        model.capabilities.includes("agent")
      ) {
        suitability += 15;
      }
      if (
        complexity.factors.messageLength > 50000 &&
        model.capabilities.includes("long-context")
      ) {
        suitability += 25;
      }
