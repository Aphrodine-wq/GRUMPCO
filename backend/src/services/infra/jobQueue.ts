...
async function dispatchServerlessJob(
  ...
  if (!baseUrl) {
    logger.error(
      { path },
      "PUBLIC_BASE_URL or VERCEL_URL is required for serverless job dispatch",
    );
    return;
  }
...
async function processOneShipJob(): Promise<boolean> {
  ...
  try {
    ...
    logger.info(
      { jobId: job.id, sessionId: job.session_id },
      "Ship job completed",
    );
  } catch (err) {
    ...
    logger.error(
      { jobId: job.id, sessionId: job.session_id, error: msg },
      "Ship job failed",
    );
  }
  return true;
...
export async function processShipJob(jobId: string): Promise<void> {
  ...
  try {
    ...
    await supabaseTable("ship_jobs")
      ...
      .eq("id", jobId);
    logger.info(
      { jobId, sessionId: data.session_id },
      "Ship job completed (serverless)",
    );
  } catch (err) {
    const msg = (err as Error).message;
    await supabaseTable("ship_jobs")
...
async function processOneExpoJob(): Promise<boolean> {
  ...
  try {
    ...
    if (result.success) {
      ...
      logger.info(
        { jobId: job.id, projectPath: job.project_path },
        "Expo test job completed",
      );
    } else {
...
export async function processCodegenJob(jobId: string): Promise<void> {
  if (!isServerlessRuntime) {
    logger.warn(
      { jobId },
      "processCodegenJob called outside serverless runtime",
    );
    return;
  }
  const now = new Date().toISOString();
  ...
  try {
    ...
    logger.info(
      { jobId, sessionId: data.session_id },
      "Codegen job completed (serverless)",
    );
  } catch (err) {
    const msg = (err as Error).message;
    await supabaseTable("codegen_jobs")
