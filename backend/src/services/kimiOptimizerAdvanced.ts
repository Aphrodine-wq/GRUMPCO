...
export function analyzeKimiCost(
  inputTokens: number,
  outputTokens: number,
  alternatives: Array<{ name: string; inputPrice: number; outputPrice: number }> = [
    { name: 'GPT-4', inputPrice: 30.0, outputPrice: 60.0 },
    { name: 'Gemini Pro', inputPrice: 3.5, outputPrice: 10.5 },
  ]
): {
  kimiCost: number;
  comparisons: Array<{ name: string; cost: number; savings: number; savingsPercent: number }>;
  cheapest: string;
  recommendation: string;
} {
...
export function getKimiRoutingDecision(input: KimiRoutingInput): {
  ...
  return {
    ...
    alternatives: [
      ...
      { provider: 'openrouter', model: 'openrouter/moonshotai/kimi-k2.5', confidence: 0.8 },
    ],
  };
}
