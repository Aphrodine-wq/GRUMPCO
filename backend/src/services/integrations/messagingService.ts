 ...
 * Now with G-Agent integration for tool execution via messaging.
...
/**
 * Get user's G-Agent preferences from database
 */
async function getUserGAgentPrefs(userId: string): Promise<{
  enabled: boolean;
...
/**
 * Process G-Agent command via messaging
 */
...
export async function processMessage(
  ...
  // Regular chat - check if G-Agent should handle it
  const userPerms = getUserPermissions(platformUserId);
  const effective = getEffectivePermissions(platform, userPerms || undefined);

  if (effective.gAgentEnabled) {
    // Route through G-Agent for tool access
    return processGAgentCommand(platform, platformUserId, text);
  }
