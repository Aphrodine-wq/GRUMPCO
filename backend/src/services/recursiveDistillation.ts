      ...
      for (const regex of prefRegexes) {
        ...
        for (const match of matches) {
          ...
          if (pattern && pattern.length > 3 && pattern.length < 200) {
            ...
            this.patternExamples.get(pattern)!.push(turn.content.slice(0, 200));
...
export class KnowledgeGraphBuilder {
  ...
  private extractEntities(text: string, timestamp: number): void {
    ...
    // Actions/verbs
    const actionPattern = /\b(create|build|fix|update|add|remove|refactor|test|deploy|implement)\s+(\w+(?:\s+\w+)?)/gi;
    let match;
   ...
   */
  getSubgraph(nodeId: string, depth: number = 2): { nodes: KnowledgeNode[]; edges: KnowledgeEdge[] } {
...
export class ModelCompressor {
   ...
   * Encode preferences
   */
  private encodePreferences(prefs: UserPreference[], embedding: Float64Array, offset: number): void {
   ...
   * Encode constraints
   */
  private encodeConstraints(constraints: Constraint[], embedding: Float64Array, offset: number): void {
   ...
   * Encode patterns
   */
  private encodePatterns(patterns: ExtractedPattern[], embedding: Float64Array, offset: number): void {
...
export class RecursiveDistiller extends EventEmitter {
  ...
  distill(userId: string = 'default'): DistillationResult {
    ...
    const userTurns = this.conversationBuffer.filter(t => 
      t.sessionId.startsWith(userId) || userId === 'default'
    );
    ...
    this.conversationBuffer = this.conversationBuffer.filter(t => 
      !(t.sessionId.startsWith(userId) || userId === 'default')
    );
    ...
    const result: DistillationResult = {
      ...
      compressionRatio: model.embedding ? 
        (model.totalTurns * 500) / (model.embedding.length * 8) : 1,
  ...
  private mergePreferences(model: UserModel, newPrefs: UserPreference[]): void {
    for (const newPref of newPrefs) {
      const existing = model.preferences.find(p => 
   ...
   */
  getKnowledgeGraph(userId: string): { nodes: KnowledgeNode[]; edges: KnowledgeEdge[] } | undefined {
  ...
  exportModel(userId: string): {
    ...
    return {
      model: {
        ...
        edges: model.edges,
        embedding: model.embedding ? Array.from(model.embedding) as unknown as Float64Array : undefined
      }
    };
  }
