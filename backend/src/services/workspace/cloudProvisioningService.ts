...
function generateTerraformGCP(spec: InfrastructureSpec): string {
  const lines: string[] = [
    ...
    ...Object.entries(spec.tags ?? {}).map(
      ([k, v]) =>
        `    ${k.toLowerCase().replace(/[^a-z0-9-]/g, "-")} = "${v.toLowerCase()}"`,
    ),
    "  }",
    "}",
    "",
  ...
  switch (resource.type) {
    ...
    case "database": {
      ...
      if (config.engine === "firestore") {
        ...
        const dbVersion =
          config.engine === "postgres" ? "POSTGRES_15" : "MYSQL_8_0";
        lines.push(
...
function generateAzureResource(
  resource: CloudResource,
  projectName: string,
): string[] {
  const lines: string[] = [];
...
function generatePulumiResource(
  resource: CloudResource,
  ...
  if (tool === "terraform") {
    ...
    files.push(
      ...
      { name: "variables.tf", content: generateTerraformVariables(spec) },
      {
        ...
        content: generateTerraformTfvars(spec),
      },
    );
  } else {
