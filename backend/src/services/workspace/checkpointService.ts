...
export class CheckpointService {
  ...
  constructor(options: Partial<CheckpointServiceOptions> & { workspaceRoot: string }) {
    ...
    this.checkpointBasePath = path.join(this.options.workspaceRoot, this.options.checkpointDir);
  }
  ...
  async createCheckpoint(
    ...
    for (let i = 0; i < snapshots.length; i++) {
      if (snapshots[i].content !== null) {
        await fs.writeFile(path.join(cpDir, `file_${i}.snapshot`), snapshots[i].content!, 'utf8');
      }
    }

    logger.info({ checkpointId: id, files: filePaths.length, description }, 'Checkpoint created');
    ...
    try {
      ...
      if (checkpoint.reverted) {
        return {
          ...
          filesRestored: 0,
          filesDeleted: 0,
          error: 'Checkpoint already reverted',
        };
      }
      ...
      logger.info({ checkpointId, filesRestored, filesDeleted }, 'Checkpoint reverted');
