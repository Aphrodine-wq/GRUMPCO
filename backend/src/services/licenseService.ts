...
import { TIERS, TierId, getTier } from '../config/pricing.js';
...
class LicenseService {
     ...
     * In production, this should verify a JWT signature or DB lookup.
     */
    async validateLicenseKey(key: string): Promise<{ valid: boolean; tier?: TierId; message?: string }> {
        const tier = tierFromKey(key);
        if (!tier) {
            return { valid: false, message: 'Invalid license key format' };
        }

        // TODO: Replace with JWT signature verification or DB lookup
        // For now, accept any correctly formatted key
