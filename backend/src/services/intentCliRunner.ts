...
export function getIntentCompilerPath(): string {
  ...
  const exe =
    process.platform === "win32" ? "grump-intent.exe" : "grump-intent";
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    child.on("close", (code: number | null) => {
      ...
      if (code !== 0) {
        ...
        killAndReject(
          `Intent compiler exited ${code}: ${stderr || "see logs"}`,
        );
        return;
      }
      try {
