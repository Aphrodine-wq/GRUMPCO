...
describe('LicenseService', () => {
  ...
  describe('validateLicenseKey', () => {
    it('accepts valid PRO key', async () => {
      const result = await licenseService.validateLicenseKey('GRUMP-PRO-abcdef1234567890');
      expect(result).toEqual({ valid: true, tier: 'pro' });
    });

    it('accepts valid TEAM key (case-insensitive)', async () => {
      const result = await licenseService.validateLicenseKey('GRUMP-TEAM-abcdef1234567890');
      expect(result).toEqual({ valid: true, tier: 'team' });
    });

    it('accepts valid ENTERPRISE key', async () => {
      const result = await licenseService.validateLicenseKey('GRUMP-ENTERPRISE-abcdef1234567890');
      expect(result).toEqual({ valid: true, tier: 'enterprise' });
    });

    it('rejects key with wrong prefix', async () => {
      const result = await licenseService.validateLicenseKey('INVALID-PRO-abcdef1234567890');
    ...
    it('rejects key with non-hex characters in hash', async () => {
      const result = await licenseService.validateLicenseKey('GRUMP-PRO-zzzzzzzzzzzzzzzz');
      expect(result.valid).toBe(false);
    });
  });
  ...
  describe('activateLicense', () => {
    it('activates a valid key', async () => {
      const ok = await licenseService.activateLicense('user-1', 'GRUMP-PRO-abcdef1234567890');
      expect(ok).toBe(true);
    });
    ...
    it('activated license is reflected in status', async () => {
      await licenseService.activateLicense('user-1', 'GRUMP-TEAM-abcdef1234567890');
