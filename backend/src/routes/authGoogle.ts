...
interface GoogleAuthCallbackQuery {
  code?: string;
  error?: string;
  state?: string;
}
 ...
 */
router.get('/', async (req: Request<unknown, unknown, unknown, GoogleAuthQuery>, res: Response) => {
  ...
  try {
    ...
    const redirectTo =
      ...
      `${process.env.PUBLIC_BASE_URL || 'http://localhost:3000'}/auth/google/callback`;
    ...
    if (error) {
      log.error({ error: error.message }, 'Google OAuth init failed');
    ...
    if (data.url) {
      // If CLI flow, append our state to the OAuth URL
      if (isCli && redirect_uri) {
    ...
    log.error({ error: error.message }, 'Google OAuth error');
    res.status(500).json({
...
router.get(
  '/callback',
  async (req: Request<unknown, unknown, unknown, GoogleAuthCallbackQuery>, res: Response) => {
    const log = getRequestLogger();
    ...
    if (oauthError) {
      log.warn({ error: oauthError }, 'Google OAuth callback error');

      if (isCli && cliRedirectUri) {
        res.redirect(
          `${cliRedirectUri}?error=${encodeURIComponent(String(oauthError))}&provider=google`
        );
        return;
      }
    ...
    if (!code) {
      ...
      if (isCli && cliRedirectUri) {
        res.redirect(`${cliRedirectUri}?error=missing_code&provider=google`);
        return;
      }
    ...
    try {
      ...
      if (error) {
        log.error({ error: error.message }, 'Google OAuth code exchange failed');

        if (isCli && cliRedirectUri) {
          res.redirect(
            `${cliRedirectUri}?error=${encodeURIComponent(error.message)}&provider=google`
          );
          return;
        }
      ...
      const error = err as Error;
      log.error({ error: error.message }, 'Google OAuth callback error');

      if (isCli && cliRedirectUri) {
        res.redirect(`${cliRedirectUri}?error=server_error&provider=google`);
        return;
      }
