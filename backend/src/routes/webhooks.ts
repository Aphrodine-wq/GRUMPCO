...
router.post(
  '/trigger',
  async (req: Request<Record<string, never>, object, TriggerBody>, res: Response) => {
    ...
    if (!action || !['ship', 'chat'].includes(action)) {
      res
        .status(400)
        .json({ error: 'Missing or invalid action (ship|chat)', type: 'validation_error' });
      return;
    }
    try {
      if (action === 'ship') {
        ...
        if (!sessionId) {
          res
            .status(400)
            .json({ error: 'ship action requires params.sessionId', type: 'validation_error' });
          return;
        }
