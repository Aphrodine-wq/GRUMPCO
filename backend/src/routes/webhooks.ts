...
interface TriggerBody {
  action: 'ship' | 'chat';
  params?: { sessionId?: string; projectDescription?: string; [k: string]: unknown };
  secret?: string;
}
...
router.post('/trigger', async (req: Request<{}, {}, TriggerBody>, res: Response) => {
  ...
  if (isProduction && !WEBHOOK_SECRET) {
    ...
    res.status(503).json({ error: 'Webhook secret not configured', type: 'config_error' });
    return;
  }
  ...
  if (!action || !['ship', 'chat'].includes(action)) {
    res.status(400).json({ error: 'Missing or invalid action (ship|chat)', type: 'validation_error' });
    return;
  }
  try {
    if (action === 'ship') {
      ...
      if (!sessionId) {
        res.status(400).json({ error: 'ship action requires params.sessionId', type: 'validation_error' });
        return;
      }
    ...
    if (action === 'chat') {
      res.status(202).json({ jobId: null, action: 'chat', message: 'Chat trigger not yet implemented; use /api/chat/stream' });
      return;
    }
    res.status(400).json({ error: 'Unknown action', type: 'validation_error' });
  } catch (e) {
