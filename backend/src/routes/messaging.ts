...
router.post('/discord', async (req: Request, res: Response) => {
  ...
  if (!token) {
    ...
    res.status(503).json({ error: 'Discord not configured', type: 'config_error' });
    return;
  }
  ...
  const text = body.content ?? body.data?.options?.[0]?.value ?? body.message?.content ?? '';
  ...
  try {
    const { sendDiscord } = await import('../services/integrations/messagingShipNotifier.js');
...
router.post('/slack', async (req: Request, res: Response) => {
  ...
  const slackToken = process.env.SLACK_BOT_TOKEN || process.env.SLACK_SIGNING_SECRET;
  if (!slackToken) {
    logger.warn('SLACK_BOT_TOKEN not set');
    res.status(503).json({ error: 'Slack not configured', type: 'config_error' });
    return;
  }
  ...
  if (body.type === 'event_callback' && body.event) {
    ...
    if (event.type === 'message' && !event.bot_id && !event.subtype && event.text) {
      ...
      const reply = await processMessage('slack', userId ?? channelId, text || '(empty)');

      // Send reply via Slack
      try {
        const { sendSlack } = await import('../services/integrations/messagingShipNotifier.js');
...
router.post('/whatsapp', async (req: Request, res: Response) => {
  ...
  const text = (body.Body ?? body.body ?? '').toString().trim();
