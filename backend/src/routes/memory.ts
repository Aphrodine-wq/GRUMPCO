...
import { z } from 'zod';
import { recall, remember, learnFromFeedback } from '../services/session/memoryService.js';
import {
  gAgentMemoryService,
  type MemoryCategory,
...
function mapMemoryType(type: string): 'interaction' | 'correction' | 'preference' {
...
router.post('/', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!type || typeof type !== 'string' || !content || typeof content !== 'string') {
      sendErrorResponse(res, ErrorCode.VALIDATION_ERROR, 'type and content required', {});
      return;
    }
    const userId =
