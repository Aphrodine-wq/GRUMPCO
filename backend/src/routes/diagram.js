...
function classifyError(error) {
    ...
    if (status === 401 || message.includes('api key') || message.includes('authentication') || message.includes('unauthorized')) {
        return {
            status: 401,
            type: 'auth_error',
            message: 'Invalid API key.',
            suggestedAction: 'Please check your ANTHROPIC_API_KEY in the backend .env file. See API_SETUP_INSTRUCTIONS.txt for help.',
            retryable: false,
        };
    }
    // Rate limiting
    ...
    if (status === 503 || status === 502 || message.includes('unavailable') || message.includes('overloaded')) {
        return {
            status: 503,
            type: 'service_unavailable',
...
router.post('/generate-diagram', validateDiagramRequest, handleValidationErrors, async (req, res) => {
    ...
    try {
        ...
        if (res.headersSent)
            return;
        log.error({ error: error.message, code: error.code }, 'Error generating diagram');
        ...
        if (classified.retryAfter) {
            res.setHeader('Retry-After', classified.retryAfter);
...
router.post('/generate-diagram-stream', validateDiagramRequest, handleValidationErrors, async (req, res) => {
    const log = getRequestLogger();
    const { message, preferences, conversationHistory, refinementContext } = req.body;
    ...
    try {
        ...
        res.flushHeaders();
        for await (const chunk of generateDiagramStream(message, preferences, abortController.signal, conversationHistory, refinementContext)) {
        ...
        log.error({ error: error.message, code: error.code }, 'Error streaming diagram');
        const classified = classifyError(error);
        const errorPayload = {
            error: classified.message,
            type: classified.type,
...
router.post('/generate-code', async (req, res) => {
    ...
    try {
        ...
        if (res.headersSent)
            return;
        log.error({ error: error.message }, 'Error generating code');
