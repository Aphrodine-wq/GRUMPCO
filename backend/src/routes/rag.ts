...
router.post("/query", async (req: Request, res: Response) => {
  try {
    ...
    if (trimmedQuery.length > MAX_QUERY_LENGTH) {
      return res
        .status(413)
        .json({ error: `query exceeds ${MAX_QUERY_LENGTH} characters` });
    }
    if (
      structuredSchema &&
    ...
    if (types !== undefined) {
      ...
      const invalid = arr.some(
        (t) => !ALLOWED_TYPES.includes(t as (typeof ALLOWED_TYPES)[number]),
      );
      if (invalid) {
        return res
          .status(400)
          .json({ error: "types must be one of doc|code|spec" });
      }
    }
    const opts = {
      ...(outputFormat && { outputFormat }),
      ...(structuredSchema &&
        typeof structuredSchema === "string" && { structuredSchema }),
    ...
    const cached = await cache.get<Awaited<ReturnType<typeof ragQuery>>>(
      "rag:query",
      key,
    );
    if (cached) {
