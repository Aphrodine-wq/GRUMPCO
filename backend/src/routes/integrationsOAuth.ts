...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      const stateToken = Buffer.from(JSON.stringify(state)).toString(
        "base64url",
      );
      ...
      log.error(
        { error: error.message, provider },
        "OAuth authorization error",
      );
      res
        .status(500)
        .json({ error: "Failed to initiate OAuth flow", provider });
    }
  },
);
...
router.get("/:provider/callback", async (req: Request, res: Response) => {
  ...
  if (oauthError) {
    ...
    res.redirect(
      `${frontendUrl}/settings?tab=integrations&error=${encodeURIComponent(String(oauthError))}`,
    );
    return;
  }
  ...
  if (!code || !state) {
    ...
    res.redirect(
      `${frontendUrl}/settings?tab=integrations&error=missing_parameters`,
    );
    return;
  }
  ...
  if (!OAUTH_PROVIDERS[provider]) {
    res.redirect(
      `${frontendUrl}/settings?tab=integrations&error=unsupported_provider`,
    );
    return;
  }
  ...
  try {
    ...
    if (!storedState || storedState !== state) {
      ...
      res.redirect(
        `${frontendUrl}/settings?tab=integrations&error=state_mismatch`,
      );
      return;
    }
    ...
    const stateData: OAuthState = JSON.parse(
      Buffer.from(String(state), "base64url").toString("utf-8"),
    );
    ...
    if (Date.now() - stateData.timestamp > 600000) {
      ...
      res.redirect(
        `${frontendUrl}/settings?tab=integrations&error=state_expired`,
      );
      return;
    }
    ...
    if (!tokenResponse.ok) {
      ...
      log.error(
        { provider, status: tokenResponse.status, error: errorText },
        "Token exchange failed",
      );
      res.redirect(
        `${frontendUrl}/settings?tab=integrations&error=token_exchange_failed`,
      );
      return;
    }
    ...
    const tokenRecord: OAuthTokenRecord = {
      ...
      refresh_token_enc: tokenData.refresh_token
        ? JSON.stringify(encrypt(tokenData.refresh_token))
        : null,
    ...
    log.info(
      { provider, userId: stateData.userId },
      "OAuth integration connected successfully",
    );
    ...
    res.redirect(
      `${frontendUrl}/settings?tab=integrations&success=${provider}`,
    );
  } catch (err) {
    const error = err as Error;
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      res.json({
        success: true,
        message:
          "Token refresh functionality ready - decrypt implementation needed",
      });
    } catch (err) {
      const error = err as Error;
