...
const PRESET_FAST = { provider: 'nim' as const, modelId: 'mistralai/mixtral-8x22b-instruct-v0.1' };
const PRESET_QUALITY = { provider: 'nim' as const, modelId: 'meta/llama-3.1-405b-instruct' };
  ...
  try {
      ...
      if (sessionType === 'gAgent') {
        ...
        if (modelPref && typeof modelPref === 'object') {
          ...
          try {
            ...
            modelPreference =
              pref && (src === 'cloud' || src === 'auto') ? { ...pref, source: src } : undefined;
          } catch {
            // ignore
          }
        }
    ...
    try {
      ...
      if (sessionType === 'gAgent') {
        ...
        gAgentCapabilities = (prefs?.gAgentCapabilities ?? prefs?.freeAgentCapabilities) as
          | string[]
          | undefined;
      ...
      if (mcpServers && Array.isArray(mcpServers) && mcpServers.length > 0) {
        ...
        await loadAllMcpTools(mcpServers as import('../types/settings.js').McpServerConfig[]);
      }
    } catch (err) {
    ...
    const textBuffer = new StreamBuffer(
      (chunk) => {
        try {
          ...
          logger.error({ error: writeError, requestId: req.id }, 'Failed to write buffered text');
        }
      },
    ...
    for await (const event of stream) {
      if (!isClientConnected) {
        logger.debug({ requestId: req.id, sessionType }, 'Client disconnected, stopping stream');
        break;
      }
    ...
    logger.debug({ requestId: req.id, sessionType }, 'Chat stream completed successfully');
  } catch (error: unknown) {
    if (!isClientConnected) {
      return;
    }

    logger.error({ error, requestId: req.id, sessionType }, 'Chat stream error');

    const err = error as { status?: number; statusCode?: number; message?: string; code?: unknown };
