...
import { runPlanCli, type Plan as RustPlan } from '../services/intentCliRunner.js';
 ...
 */
router.post('/generate-rust', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logger.debug({ requestId: req.id, goalLength: goal.length }, 'G-Agent plan generation request');
    ...
    if (matchedPattern) {
      ...
      plan = {
        ...
        tasks: matchedPattern.tasks.map((pt, idx) => ({
          ...
          blocks: idx < matchedPattern.tasks.length - 1 ? [`task_${idx + 2}`] : [],
        ...
        execution_order: matchedPattern.tasks.map((_, idx) => `task_${idx + 1}`),
        parallel_batches: matchedPattern.tasks.map((_, idx) => [`task_${idx + 1}`]),
    ...
    logger.error({ error, requestId: req.id }, 'G-Agent plan generation failed');
    sendServerError(res, error);
  }
});
 ...
 */
router.post('/execute-rust', async (req: Request, res: Response): Promise<void> => {
  const { plan, workspaceRoot } = req.body as { plan?: RustPlan; workspaceRoot?: string };
  ...
  try {
    ...
    logger.error({ error: message, requestId: req.id, planId: plan.id }, 'Plan execution error');
 ...
 */
router.post('/:id/cancel-execution', async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params as { id: string };
    ...
    logger.info({ requestId: req.id, planId: id }, 'Plan execution cancelled');
 ...
 */
router.get('/:id/execution-status', async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params as { id: string };
