...
function getFigmaConfig(): {
  ...
  const clientSecret =
    process.env.FIGMA_CLIENT_SECRET ?? env.FIGMA_CLIENT_SECRET;
  const redirectUri =
    process.env.FIGMA_REDIRECT_URI ??
    env.FIGMA_REDIRECT_URI ??
    `${env.PUBLIC_BASE_URL ?? "http://localhost:3000"}/api/figma/callback`;
...
router.get("/callback", async (req: Request, res: Response): Promise<void> => {
  ...
  if (!code) {
    res
      .status(400)
      .send(
        "<html><body><p>Missing code</p><script>window.close()</script></body></html>",
      );
    return;
  }
  const config = getFigmaConfig();
  if (!config) {
    res
      .status(503)
      .send(
        "<html><body><p>Figma not configured</p><script>window.close()</script></body></html>",
      );
    return;
  }
  try {
    ...
    if (!tokenRes.ok) {
      ...
      res
        .status(400)
        .send(
          `<html><body><p>Token exchange failed</p><script>window.close()</script></body></html>`,
        );
      return;
    }
    ...
    res
      .status(500)
      .send(
        "<html><body><p>Error connecting Figma</p><script>window.close()</script></body></html>",
      );
    return;
  }
});
  ...
  try {
    ...
    if (!apiRes.ok) {
      ...
      res
        ...
        .json({ error: "Figma API error", details: errText });
      return;
    }
