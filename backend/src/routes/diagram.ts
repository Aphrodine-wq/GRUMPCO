...
function classifyError(error: ErrorWithStatus): ClassifiedError {
  ...
  if (code === "CIRCUIT_OPEN") {
    return {
      ...
      suggestedAction:
        "The service is temporarily paused to recover. Try again in a minute.",
      retryable: true,
  ...
  if (code === "EXTRACTION_FAILED") {
    return {
      ...
      suggestedAction:
        "Try rephrasing your request or be more specific about what you want.",
      retryable: true,
    };
  }
  ...
  async (req: DiagramRequestBody, res: Response) => {
    ...
    if (clarificationAnswers?.answers?.length) {
      ...
      log.info(
        ...
        "Processing clarification answers",
      );
    }
    ...
    const timeoutId = setTimeout(() => {
      if (!res.writableEnded) {
        ...
        if (res.headersSent) {
          res.write(
            `data: ${JSON.stringify({ error: "Request timeout", type: "timeout" })}\n\n`,
          );
          res.end();
        } else {
...
router.post(
  "/generate-code",
  async (req: CodeGenRequestBody, res: Response) => {
    const log = getRequestLogger();
    ...
    try {
      ...
      zipStream.on("error", (err: Error) => {
        ...
        if (!res.headersSent) {
          res
            .status(500)
            .json({ error: "Failed to generate ZIP", type: "internal_error" });
        }
      });
    } catch (error) {
      clearTimeout(timeoutId);
