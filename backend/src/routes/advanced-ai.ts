...
import { RecursiveDistillationService, type ConversationTurn } from '../services/recursiveDistillation.js';
...
router.post('/context/similarity', async (req: Request, res: Response) => {
  try {
    ...
    if (!id1 || !id2) {
      return res.status(400).json({ error: 'Either contextId1/contextId2 or text1/text2 are required' });
    }
 ...
 */
router.get('/swarm/status/:swarmId?', async (req: Request, res: Response) => {
  try {
    const swarmId = req.params.swarmId || 'default';
...
router.post('/swarm/inject', async (req: Request, res: Response) => {
  try {
    const { swarmId, content, confidence = 0.8, tags = [] } = req.body as { 
      swarmId?: string; 
      content?: string;
      confidence?: number;
      tags?: string[];
 ...
 */
router.post('/swarm/stop/:swarmId?', async (req: Request, res: Response) => {
  try {
    const swarmId = req.params.swarmId || 'default';
...
router.post('/supervised/create', async (req: Request, res: Response) => {
  try {
    const { swarmId, agentsPerRole = 1, reviewStrictness = 'moderate' } = req.body as { 
      swarmId?: string; 
    ...
    logger.error({ error: (e as Error).message }, 'Supervised swarm create error');
    return res.status(500).json({ error: (e as Error).message });
  }
});
  ...
  try {
    ...
    return res.json({
      ...
      task: {
        ...
        approvedCount: task.subtasks.filter(st => st.status === 'approved').length,
        rejectedCount: task.subtasks.filter(st => st.status === 'rejected').length,
        synthesizedResult: task.synthesizedResult,
...
router.get('/supervised/task/:taskId', async (req: Request, res: Response) => {
  try {
    ...
    logger.error({ error: (e as Error).message }, 'Supervised task status error');
    return res.status(500).json({ error: (e as Error).message });
  }
});
 ...
 */
router.get('/supervised/status/:swarmId?', async (req: Request, res: Response) => {
  try {
    const swarmId = req.params.swarmId || 'default';
 ...
 */
router.get('/distill/model/:userId?', async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/distill/knowledge/:userId?', async (req: Request, res: Response) => {
  try {
    const userId = req.params.userId || 'default';
