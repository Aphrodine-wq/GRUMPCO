...
router.post('/context/similarity', async (req: Request, res: Response) => {
  try {
    ...
    if (!id1 || !id2) {
      return res
        .status(400)
        .json({ error: 'Either contextId1/contextId2 or text1/text2 are required' });
    }
 ...
 */
router.get(['/swarm/status', '/swarm/status/:swarmId'], async (req: Request, res: Response) => {
  try {
 ...
 */
router.post(['/swarm/stop', '/swarm/stop/:swarmId'], async (req: Request, res: Response) => {
  try {
    const { swarmId: swarmIdParam } = req.params as { swarmId?: string };
    const swarmId = swarmIdParam || 'default';
 ...
 */
router.get(['/distill/model', '/distill/model/:userId'], async (req: Request, res: Response) => {
  try {
