    ...
    tools: [
        {
            ...
            description: 'Analyze code for performance bottlenecks, expensive operations, and optimization opportunities',
            input_schema: {
                type: 'object',
                properties: {
                    code: { type: 'string', description: 'Source code to analyze' },
                    language: { type: 'string', enum: ['typescript', 'javascript', 'python', 'go'], description: 'Programming language' },
                    focus: { type: 'string', enum: ['cpu', 'memory', 'rendering', 'all'], description: 'Analysis focus area' },
                },
                required: ['code'],
            },
        },
        {
            ...
            input_schema: {
                type: 'object',
                properties: {
                    imports: { type: 'string', description: 'Import statements or package.json dependencies' },
        ...
        },
        {
            ...
            description: 'Generate actionable optimization recommendations with before/after code examples',
            input_schema: {
                type: 'object',
                properties: {
    ...
    async execute(toolName: string, input: Record<string, unknown>): Promise<string> {
        switch (toolName) {
            case 'analyze_performance': {
                ...
                return `Performance analysis (${language || 'auto-detected'}, focus: ${focus || 'all'}):\n` +
            ...
            case 'check_bundle_size': {
                ...
                return `Bundle analysis (threshold: ${threshold || 250}KB):\n` +
                    `Imports analyzed:\n${imports}\n\n` +
            ...
            case 'suggest_optimizations': {
                ...
                return `Optimization suggestions (target: ${target || 'both'}):\n` +
                    `Code analyzed:\n${code.substring(0, 300)}...\n\n` +
