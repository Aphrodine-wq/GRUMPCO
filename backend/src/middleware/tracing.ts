...
async function loadOtelModules(): Promise<void> {
  ...
  const [sdkMod, autoInstrMod, resourceMod, semconvMod, otlpMod, traceMod] =
    await Promise.all([
...
export function initializeTracing(): void {
  ...
  if (!shouldEnableTracing()) {
    logger.debug(
      "OpenTelemetry tracing disabled (serverless/dev mode without OTLP_ENDPOINT)",
    );
    isInitialized = true;
    return;
  }
  ...
  initPromise = (async () => {
    try {
      ...
      const otlpEndpoint =
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT || process.env.OTLP_ENDPOINT;
...
export function tracingMiddleware(
  ...
  const correlationId =
    getCorrelationIdFromHeaders(req.headers) || generateCorrelationId();
  ...
  (req as Request & { span?: Span; correlationId?: string }).correlationId =
    correlationId;
