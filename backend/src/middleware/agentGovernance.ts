...
import { createApprovalRequest, getApprovalRequest } from '../services/security/approvalService.js';
...
export async function agentGovernanceMiddleware(
  ...
  if (policy === 'audit_only') {
    ...
    log.info({ agentId, path: req.path }, 'Agent request (audit_only, allowed)');
    next();
    return;
  }
  ...
  if (!isAllowlisted(agentId)) {
    ...
    res.status(403).json({
      error: 'Agent not in allowlist. Add your agent ID to AGENT_ALLOWLIST to permit access.',
      type: 'agent_blocked',
  ...
  if (isRiskyPath(req.path)) {
    ...
    try {
      ...
      await recordAgentAudit(agentId, req.path, 'approval_required', 'approval_required');
      log.info({ agentId, path: req.path, approvalId: approval.id }, 'Agent approval required');
      res.status(202).json({
      ...
      log.error({ error: (err as Error).message }, 'Failed to create agent approval');
      res.status(500).json({
