...
export function recordContextGeneration(
  duration: number,
  status: "success" | "error",
): void {
...
export function updateWorkerPoolMetrics(
  queueDepth: number,
  utilization: number,
): void {
...
export function updateGpuMetrics(
  gpuId: string,
  utilization: number,
  memoryUsed: number,
): void {
...
export function recordLlmRouterSelection(
  provider: string,
  modelId: string,
): void {
...
export function recordLlmStreamMetrics(
  ...
  if (
    typeof timeToFirstTokenSeconds === "number" &&
    timeToFirstTokenSeconds >= 0
  ) {
    llmTimeToFirstTokenSeconds.observe(
      { provider, model },
