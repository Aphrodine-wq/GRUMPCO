...
4. Match patterns from memory when applicable`,
  ...
  executor: {
    ...
    tools: ['file_read', 'file_write', 'file_edit', 'bash_execute', 'codebase_search'],
...
4. Handle authentication and authorization`,
  ...
  devops: {
    ...
    type: 'devops',
    name: 'DevOps',
    description: 'Creates CI/CD pipelines, Docker configs, and deployment scripts',
    capabilities: ['devops', 'ci', 'deployment'],
    tools: ['file_read', 'file_write', 'bash_execute', 'docker_ps', 'docker_compose_up'],
...
export class AgentRegistry {
  ...
  findByTool(tool: string): AgentDefinition[] {
    return Array.from(this.agents.values()).filter((a) => a.tools.includes(tool));
  }
  ...
  selectAgentsForTask(intent: string, tier: AgentTier): AgentType[] {
    ...
    if (keywords.includes('plan') || keywords.includes('build') || keywords.includes('create')) {
    ...
    for (const [keyword, agents] of Object.entries(keywordMap)) {
      if (keywords.includes(keyword)) {
        for (const agent of agents) {
          if (available.some((a) => a.type === agent) && !selected.includes(agent)) {
            selected.push(agent);
          }
        }
      }
