...
class ChaosMonkey {
  ...
  shouldApply(service?: string, endpoint?: string): boolean {
    ...
    if (
      endpoint &&
      this.config.excludeEndpoints.some((e) => endpoint.startsWith(e))
    ) {
      return false;
    }
  ...
  async maybeInjectLatency(service?: string, endpoint?: string): Promise<void> {
    ...
    if (Math.random() < this.config.latencyProbability) {
      ...
      logger.warn(
        { service, endpoint, delayMs: delay },
  ...
  getStats(): {
    ...
    const latencyEvents = this.events.filter(
      (e) => e.type === "latency",
    ).length;
...
export function applyChaosScenario(
  ...
  chaos.configure({
    ...
    errorTypes: [...config.errorTypes] as (
      | "timeout"
      | "network"
      | "server"
      | "rate_limit"
