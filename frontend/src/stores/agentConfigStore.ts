...
const initialState: agentConfigStoreState = {
  config: null,
  features: [],
  presets: [],
  isLoading: false,
...
const store = writable<agentConfigStoreState>(initialState);
...
export const agentConfigStore = {
   ...
   */
  async fetchConfig(sessionId: string = 'default'): Promise<AgentConfig | null> {
   ...
   * Get current state
   */
  getState(): agentConfigStoreState {
    return get(store);
  },
