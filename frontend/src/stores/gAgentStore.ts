...
// Store state
interface GAgentStoreState {
  goals: Goal[];
  queueStats: QueueStats | null;
  globalStopInfo: GlobalStopInfo;
  status: GAgentStatus | null;
  isLoading: boolean;
  isConnected: boolean;
  error: string | null;
  lastEvent: GAgentSSEEvent | null;
}
...
const initialState: GAgentStoreState = {
  goals: [],
  queueStats: null,
...
const store = writable<GAgentStoreState>(initialState);
...
type SSEEventListener = (event: GAgentSSEEvent) => void;
...
export const gAgentStore = {
   ...
   */
  emitSSEEvent(event: GAgentSSEEvent): void {
  ...
  connect(sessionId: string = 'default'): void {
    ...
    const url = `${getApiBase()}/api/gagent/stream?sessionId=${encodeURIComponent(sessionId)}`;
    ...
    for (const eventType of eventTypes) {
      eventSource.addEventListener(eventType, (e: MessageEvent) => {
        try {
          ...
          const event: GAgentSSEEvent = { type: eventType as GAgentSSEEvent['type'], ...data };
   ...
   */
  handleSSEEvent(event: GAgentSSEEvent): void {
    switch (event.type) {
      case 'goal_created':
        store.update((s) => ({
          ...s,
  ...
  async fetchGoals(statusFilter?: GoalStatus[]): Promise<Goal[]> {
    ...
    try {
      ...
      const response = await fetchApi(`/api/gagent/goals${params}`);
  ...
  async createGoal(goal: {
    ...
    try {
      const response = await fetchApi('/api/gagent/goals', {
        method: 'POST',
  ...
  async cancelGoal(goalId: string): Promise<Goal> {
    try {
      const response = await fetchApi(`/api/gagent/goals/${goalId}/cancel`, {
        method: 'POST',
      });
  ...
  async retryGoal(goalId: string): Promise<Goal> {
    try {
      const response = await fetchApi(`/api/gagent/goals/${goalId}/retry`, {
        method: 'POST',
      });
  ...
  async fetchQueueStats(): Promise<QueueStats> {
    try {
      const response = await fetchApi('/api/gagent/queue/stats');
  ...
  async startQueue(): Promise<void> {
    try {
      const response = await fetchApi('/api/gagent/queue/start', {
        method: 'POST',
      });
  ...
  async stopQueue(): Promise<void> {
    try {
      const response = await fetchApi('/api/gagent/queue/stop', {
        method: 'POST',
      });
  ...
  async emergencyStop(reason?: string): Promise<void> {
    try {
      const response = await fetchApi('/api/gagent/control/stop', {
        method: 'POST',
  ...
  async stopGoal(goalId: string, reason?: string): Promise<void> {
    try {
      const response = await fetchApi(`/api/gagent/control/stop/goal/${goalId}`, {
        method: 'POST',
        body: JSON.stringify({ reason }),
      });
  ...
  async resumeOperations(): Promise<void> {
    try {
      const response = await fetchApi('/api/gagent/control/resume', {
        method: 'POST',
      });
  ...
  async fetchControlStatus(): Promise<void> {
    try {
      const response = await fetchApi('/api/gagent/control/status');
  ...
  async fetchStatus(sessionId?: string): Promise<GAgentStatus> {
    ...
    try {
      ...
      const response = await fetchApi(`/api/gagent/status${params}`);

      if (!response.ok) {
        throw new Error(`Failed to fetch status: ${response.status}`);
      }

      const status = (await response.json()) as GAgentStatus;
   ...
   * Get current state
   */
  getState(): GAgentStoreState {
    return get(store);
  },
};

export default gAgentStore;
