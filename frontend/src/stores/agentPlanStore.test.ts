...
describe('agentPlanStore', () => {
  ...
  describe('setPlan', () => {
    it('should set the current plan', async () => {
      const { agentPlanStore, currentPlan } = await import('./agentPlanStore');
      const plan = createMockPlan();

      agentPlanStore.setPlan(plan);
    ...
    it('should clear any existing error', async () => {
      const { agentPlanStore, planError } = await import('./agentPlanStore');

      // Set an error first
      agentPlanStore.finishGenerating(undefined, 'Some error');
      expect(get(planError)).toBe('Some error');

      // Now set a plan
      agentPlanStore.setPlan(createMockPlan());
      expect(get(planError)).toBeNull();
    });
  });
  ...
  describe('startGenerating / finishGenerating', () => {
    it('should set isGenerating to true', async () => {
      const { agentPlanStore, isGenerating } = await import('./agentPlanStore');

      agentPlanStore.startGenerating();
    ...
    it('should clear error when starting', async () => {
      ...
      agentPlanStore.finishGenerating(undefined, 'Old error');
      agentPlanStore.startGenerating();
    ...
    it('should finish with plan', async () => {
      const { agentPlanStore, isGenerating, currentPlan } = await import('./agentPlanStore');

      agentPlanStore.startGenerating();
      const plan = createMockPlan();
      agentPlanStore.finishGenerating(plan);
    ...
    it('should finish with error', async () => {
      const { agentPlanStore, isGenerating, planError } = await import('./agentPlanStore');

      agentPlanStore.startGenerating();
      agentPlanStore.finishGenerating(undefined, 'Generation failed');
  ...
  describe('updateTaskStatus', () => {
    it('should update task status to in_progress', async () => {
      ...
      agentPlanStore.setPlan(createMockPlan());
      agentPlanStore.updateTaskStatus('task-1', 'in_progress');
    ...
    it('should update task status to completed with output', async () => {
      ...
      agentPlanStore.setPlan(createMockPlan());
      agentPlanStore.updateTaskStatus('task-1', 'completed', 'Task output');
    ...
    it('should update task status to failed with error', async () => {
      ...
      agentPlanStore.setPlan(createMockPlan());
      agentPlanStore.updateTaskStatus('task-1', 'failed', undefined, 'Task error');
    ...
    it('should mark plan as completed when all tasks complete', async () => {
      ...
      agentPlanStore.setPlan(createMockPlan());
      agentPlanStore.updateTaskStatus('task-1', 'completed');
      agentPlanStore.updateTaskStatus('task-2', 'completed');
    ...
    it('should mark plan as failed if any task fails', async () => {
      ...
      agentPlanStore.setPlan(createMockPlan());
      agentPlanStore.updateTaskStatus('task-1', 'completed');
      agentPlanStore.updateTaskStatus('task-2', 'failed');
  ...
  describe('updateReadyTasks', () => {
    it('should approve tasks whose dependencies are complete', async () => {
      ...
      agentPlanStore.setPlan(createMockPlan());
      agentPlanStore.updateTaskStatus('task-1', 'completed');
      agentPlanStore.updateReadyTasks();
  ...
  describe('clearPlan', () => {
    it('should clear the current plan', async () => {
      ...
      agentPlanStore.setPlan(createMockPlan());
      agentPlanStore.clearPlan();

      expect(get(currentPlan)).toBeNull();
    });

    it('should add plan to history', async () => {
      const { agentPlanStore } = await import('./agentPlanStore');

      agentPlanStore.setPlan(createMockPlan());
      agentPlanStore.clearPlan();

      const state = get(agentPlanStore);
    ...
    it('should keep only last 10 plans in history', async () => {
      ...
      for (let i = 0; i < 15; i++) {
        agentPlanStore.setPlan(createMockPlan({ id: `plan-${i}` }));
        agentPlanStore.clearPlan();
      }
  ...
  describe('getPlan', () => {
    it('should return null when no plan', async () => {
      ...
      expect(agentPlanStore.getPlan()).toBeNull();
    });
    ...
    it('should return current plan', async () => {
      ...
      expect(agentPlanStore.getPlan()?.id).toBe('plan-123');
    });
  });
  ...
  describe('execution event listeners', () => {
    it('should add and remove listeners', async () => {
      ...
      const unsubscribe = agentPlanStore.onExecutionEvent(listener);

      agentPlanStore.emitExecutionEvent({ type: 'done' });
    ...
    it('should handle listener errors gracefully', async () => {
      ...
      agentPlanStore.onExecutionEvent(errorListener);
      agentPlanStore.onExecutionEvent(goodListener);

      // Should not throw
      expect(() => agentPlanStore.emitExecutionEvent({ type: 'done' })).not.toThrow();
  ...
  describe('handleExecutionEvent', () => {
    it('should handle plan_started event', async () => {
      ...
      agentPlanStore.setPlan(createMockPlan());
      agentPlanStore.handleExecutionEvent({
        type: 'plan_started',
        planId: 'plan-123',
        goal: 'Build app',
        taskCount: 2,
    ...
    it('should handle plan_completed event', async () => {
      const { agentPlanStore, currentPlan, isExecuting } = await import('./agentPlanStore');
  ...
  describe('derived stores', () => {
    it('readyTasks should return tasks with completed dependencies', async () => {
      const { agentPlanStore, readyTasks } = await import('./agentPlanStore');
    ...
    it('executionProgress should track completion', async () => {
      const { agentPlanStore, executionProgress } = await import('./agentPlanStore');
  ...
  describe('executeCurrentPlan', () => {
    it('should throw if no plan exists', async () => {
      ...
      await expect(agentPlanStore.executeCurrentPlan()).rejects.toThrow('No plan to execute');
    });

    it('should execute plan and handle SSE stream', async () => {
      const { agentPlanStore, isExecuting, currentPlan } = await import('./agentPlanStore');
      ...
      await agentPlanStore.executeCurrentPlan();
    ...
    it('should handle invalid JSON in SSE stream', async () => {
      ...
      // Should not throw
      await expect(agentPlanStore.executeCurrentPlan()).resolves.not.toThrow();
  ...
  describe('cancelExecution', () => {
    it('should abort the execution controller', async () => {
      ...
      await agentPlanStore.cancelExecution();
    ...
    it('should handle cancel API failure gracefully', async () => {
      ...
      // Should not throw
      await expect(agentPlanStore.cancelExecution()).resolves.not.toThrow();
    ...
    it('should abort the SSE connection when called during execution', async () => {
      ...
      agentPlanStore.executeCurrentPlan();
  ...
  describe('generatePlan', () => {
    it('should generate a plan successfully', async () => {
      const { agentPlanStore, currentPlan, isGenerating } = await import('./agentPlanStore');
      const { fetchApi } = await import('../lib/api.js');
      const mockFetchApi = fetchApi as ReturnType<typeof vi.fn>;
      ...
      const result = await agentPlanStore.generatePlan('Build a todo app');
    ...
    it('should handle generation API error', async () => {
      ...
      await expect(agentPlanStore.generatePlan('Build app')).rejects.toThrow('Server error');
      expect(get(isGenerating)).toBe(false);
    ...
    it('should handle network error during generation', async () => {
      ...
      await expect(agentPlanStore.generatePlan('Build app')).rejects.toThrow('Network error');
      expect(get(isGenerating)).toBe(false);
    });
    ...
    it('should initialize task statuses to pending', async () => {
      ...
      await agentPlanStore.generatePlan('Build app');
