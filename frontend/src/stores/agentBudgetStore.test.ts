...
let agentBudgetStore: typeof import('./agentBudgetStore').agentBudgetStore;
let budgetStatus: typeof import('./agentBudgetStore').budgetStatus;
let isWarning: typeof import('./agentBudgetStore').isWarning;
let isExceeded: typeof import('./agentBudgetStore').isExceeded;
let canProceed: typeof import('./agentBudgetStore').canProceed;
let pendingApprovalCount: typeof import('./agentBudgetStore').pendingApprovalCount;
let formatCost: typeof import('./agentBudgetStore').formatCost;
let getStatusColor: typeof import('./agentBudgetStore').getStatusColor;
let getStatusIcon: typeof import('./agentBudgetStore').getStatusIcon;
let fetchApi: typeof import('../lib/api.js').fetchApi;

describe('agentBudgetStore', () => {
  beforeAll(async () => {
    const storeMod = await import('./agentBudgetStore');
    const apiMod = await import('../lib/api.js');
    agentBudgetStore = storeMod.agentBudgetStore;
  ...
  describe('initial state', () => {
    it('should have null status initially', () => {
      const state = agentBudgetStore.getState();
  ...
  describe('derived stores', () => {
    it('budgetStatus should reflect store state', async () => {
      ...
      await agentBudgetStore.fetchStatus();
    ...
    it('pendingApprovalCount should count pending approvals', async () => {
      agentBudgetStore.handleBudgetEvent({
  ...
  describe('fetchStatus', () => {
    it('should fetch and store budget status', async () => {
      ...
      const result = await agentBudgetStore.fetchStatus('test-session');

      expect(mockFetchApi).toHaveBeenCalledWith('/api/gagent/budget/status?sessionId=test-session');
      expect(result).toEqual(mockStatus);
      expect(agentBudgetStore.getState().sessionCost).toBe(50);
    });
    ...
    it('should handle fetch error', async () => {
      ...
      const result = await agentBudgetStore.fetchStatus();

      expect(result).toBeNull();
      expect(agentBudgetStore.getState().error).toBe('Failed to fetch budget status: 500');
    });
  });
  ...
  describe('fetchConfig', () => {
    it('should fetch and store budget config', async () => {
      ...
      const result = await agentBudgetStore.fetchConfig('test-session');

      expect(result).toEqual(mockConfig);
      expect(agentBudgetStore.getState().config).toEqual(mockConfig);
    });
  });
  ...
  describe('updateConfig', () => {
    it('should update budget config', async () => {
      ...
      const result = await agentBudgetStore.updateConfig({ sessionLimit: 2000 });
    ...
    it('should handle update error', async () => {
      ...
      const result = await agentBudgetStore.updateConfig({ sessionLimit: -1 });

      expect(result).toBeNull();
      expect(agentBudgetStore.getState().error).toBe('Invalid config');
    });
  });
  ...
  describe('estimateCost', () => {
    it('should estimate cost for operations', async () => {
      ...
      const result = await agentBudgetStore.estimateCost(operations);

      expect(result).toEqual(mockEstimate);
      expect(agentBudgetStore.getState().lastEstimate).toEqual(mockEstimate);
    });
    ...
    it('should handle estimation error', async () => {
      ...
      const result = await agentBudgetStore.estimateCost([]);

      expect(result).toBeNull();
      expect(agentBudgetStore.getState().error).toBe('Cost estimation failed: 500');
    });
  });

  describe('quickEstimate', () => {
    it('should calculate quick estimate for claude-3.5-sonnet', async () => {
      const result = agentBudgetStore.quickEstimate(1000, 500, 'claude-3.5-sonnet');
    ...
    it('should calculate quick estimate for gpt-4o-mini', async () => {
      const result = agentBudgetStore.quickEstimate(10000, 5000, 'gpt-4o-mini');
    ...
    it('should format dollars for amounts >= 100 cents', async () => {
      const result = agentBudgetStore.quickEstimate(1000000, 500000, 'claude-3-opus');
  ...
  describe('canAfford', () => {
    it('should check if operation is affordable', async () => {
      ...
      const result = await agentBudgetStore.canAfford(50);
    ...
    it('should return reason when not affordable', async () => {
      ...
      const result = await agentBudgetStore.canAfford(1000);
  ...
  describe('session management', () => {
    it('should start session', async () => {
      ...
      const result = await agentBudgetStore.startSession('test-session');
    ...
    it('should end session and return summary', async () => {
      ...
      const result = await agentBudgetStore.endSession('test-session');
  ...
  describe('approveRequest', () => {
    it('should approve pending request', async () => {
      ...
      const result = await agentBudgetStore.approveRequest('approval-123');
    ...
    it('should handle approve failure', async () => {
      ...
      const result = await agentBudgetStore.approveRequest('invalid');
  ...
  describe('denyRequest', () => {
    it('should deny pending request', async () => {
      ...
      const result = await agentBudgetStore.denyRequest('approval-123');
  ...
  describe('dismissWarning', () => {
    it('should dismiss warning', async () => {
      ...
      expect(agentBudgetStore.getState().showWarning).toBe(true);

      // Dismiss
      agentBudgetStore.dismissWarning();

      expect(agentBudgetStore.getState().showWarning).toBe(false);
      expect(agentBudgetStore.getState().warningMessage).toBeNull();
    });
  });
