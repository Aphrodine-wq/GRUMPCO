...
function parseMermaidDiagram(code: string): ParsedDiagram | null {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (line.startsWith('%%') || line.startsWith('style ') || line.startsWith('classDef ') ||
            line.startsWith('class ') || line.startsWith('click ') || line.startsWith('linkStyle ') ||
            line.startsWith('subgraph') || line === 'end' || line.startsWith('direction ')) {
            continue;
        }
...
function drawBox(label: string, shape: ParsedNode['shape'], width: number): string[] {
    ...
    const paddedLabel = label.length > innerWidth
        ? label.slice(0, innerWidth - 1) + '…'
        : label;
                ...
                for (let h = 0; h < lineHeight; h++) {
                    ...
                    for (const conn of connections) {
                        ...
                        if (center < chars.length) {
                            chars[center] =
                                conn.style === 'dotted' ? BOX.dottedV : BOX.vertical;
                        }
...
function renderHorizontalDiagram(diagram: ParsedDiagram): string {
    ...
    const totalCols = layers.reduce((sum, _, i) => sum + layerWidths[i], 0) +
        (layers.length - 1) * ARROW_WIDTH;
...
function parseSequenceDiagram(code: string): { participants: SeqParticipant[]; messages: SeqMessage[] } | null {
    const lines = code.trim().split('\n');
...
function renderSequenceDiagram(code: string): string | null {
    ...
    for (const p of participants) {
        const centered = p.label.length > COL_WIDTH - 4
            ? p.label.slice(0, COL_WIDTH - 5) + '…'
            : p.label;
        ...
        headerMid += BOX.vertical + ' '.repeat(lpad + 1) + centered + ' '.repeat(rpad + 1) + BOX.vertical;
    ...
    for (const msg of messages) {
        ...
        for (let x = 0; x < totalWidth; x++) {
            ...
            for (const [, col] of colPositions) {
                if (x === col) { isLifeline = true; break; }
            }
            lifeline += isLifeline ? '│' : ' ';
        }
...
function parseClassDiagram(code: string): ClassDef[] | null {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        // Relationship: A --|> B
        const relMatch = line.match(/^(\w+)\s*(?:<\|--|--\|>|<\|\.\.|\.\.\|>|\*--|--\*|o--|--o|<--|-->|\.\.>|<\.\.)\s*(\w+)/);
        if (relMatch) {
...
function renderClassDiagram(code: string): string | null {
    ...
    for (const cls of classes) {
        ...
        output.push(`${BOX.vertical} ${' '.repeat(nameLpad)}${cls.name}${' '.repeat(nameRpad)} ${BOX.vertical}`);
        ...
        if (cls.methods.length > 0) {
            ...
            for (const method of cls.methods) {
                ...
                output.push(`${BOX.vertical} ${method}${' '.repeat(Math.max(0, methodPad))} ${BOX.vertical}`);
            }
        }
...
function parseStateDiagram(code: string): { states: string[]; transitions: { from: string; to: string; label: string }[] } | null {
    const lines = code.trim().split('\n');
    const first = lines[0].trim();
...
function renderStateDiagram(code: string): string | null {
    ...
    for (let i = 0; i < states.length; i++) {
        ...
        if (state === '●') {
            ...
            output.push(`${BOX.roundBottomLeft}${BOX.horizontal.repeat(boxWidth)}${BOX.roundBottomRight}`);
        }
...
function parseERDiagram(code: string): { entities: EREntity[]; relations: ERRelation[] } | null {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        const relMatch = line.match(/^(\w+)\s+(\|{1,2}|}{1,2}|o)(-{1,2})(o|\|{1,2}|}{1,2}|\{)\s+(\w+)\s*:\s*(.+)$/);
        if (relMatch) {
...
function renderERDiagram(code: string): string | null {
    ...
    for (const entity of entities) {
        ...
        output.push(`${BOX.topLeft}${divider}${BOX.topRight}`);
        output.push(`${BOX.vertical} ${' '.repeat(nlpad)}${entity.name}${' '.repeat(nrpad)} ${BOX.vertical}`);
    ...
    if (relations.length > 0) {
        ...
        for (const rel of relations) {
            output.push(`  ${rel.from} ${rel.fromCardinality}──${rel.toCardinality} ${rel.to} : ${rel.label}`);
        }
    }
...
function parseGanttChart(code: string): { title: string; sections: { name: string; tasks: GanttTask[] }[] } | null {
    const lines = code.trim().split('\n');
    const first = lines[0].trim();
...
function renderGanttChart(code: string): string | null {
    ...
    for (const section of sections) {
        ...
        output.push(`${BOX.vertical}  ${section.name}${' '.repeat(Math.max(0, secPad))}  ${BOX.vertical}`);
        ...
        for (const task of section.tasks) {
            ...
            let bar = ' '.repeat(barOffset) + barChar.repeat(barLen) + ' '.repeat(Math.max(0, BAR_WIDTH - barOffset - barLen));

            const rowPad = totalWidth - namePart.length - bar.length - 6;
            output.push(`${BOX.vertical}  ${namePart}  ${bar}${' '.repeat(Math.max(0, rowPad))}  ${BOX.vertical}`);
            taskIndex++;
        }
        output.push(`${BOX.vertical}${' '.repeat(totalWidth)}${BOX.vertical}`);
    }
...
function parsePieChart(code: string): { title: string; slices: { label: string; value: number }[] } | null {
    const lines = code.trim().split('\n');
    const first = lines[0].trim();
...
function formatUnsupportedDiagram(code: string): string {
    ...
    output.push(`${BOX.topLeft}${divider}${BOX.topRight}`);
    output.push(
        `${BOX.vertical}  ${title}${' '.repeat(maxLen - title.length - 2)}${BOX.vertical}`
    );
    output.push(`${BOX.teeRight}${divider}${BOX.teeLeft}`);
...
export function canRenderAsAscii(code: string): boolean {
    ...
    if (/^(sequenceDiagram|classDiagram|stateDiagram|erDiagram|gantt|pie)/i.test(firstLine)) return true;
    return false;
}
