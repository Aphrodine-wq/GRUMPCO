...
export type IntegrationProvider =
  | 'discord' | 'slack' | 'spotify' | 'obsidian' | 'gmail'
  | 'google_calendar' | 'notion' | 'twitter' | 'github'
  | 'linear' | 'figma' | 'home_assistant' | 'elevenlabs'
  | 'twilio' | 'sendgrid' | 'stripe' | 'custom';
...
export async function updateHeartbeat(
  id: string,
  updates: { name?: string; cronExpression?: string; enabled?: boolean; payload?: Record<string, unknown> }
): Promise<Heartbeat> {
  const res = await fetchApi(`/api/heartbeats/${id}`, {
    method: 'PATCH',
    body: JSON.stringify(updates),
