 ...
 * 
 * Converted from TypeScript to CommonJS for Electron main process
 */
...
// Docker client instance (lazy loaded)
let docker = null;
let Dockerode = null;
  ...
  if (!docker) {
    ...
    if (!Dockerode) {
      try {
        ...
        console.warn('[Docker] dockerode not installed. Run: npm install dockerode');
        return null;
      }
    }
      ...
      if (container.State === 'running') {
        try {
          ...
          stats = {
            ...
            networkRx: Object.values(statsStream.networks || {}).reduce((sum, n) => sum + (n.rx_bytes || 0), 0),
            networkTx: Object.values(statsStream.networks || {}).reduce((sum, n) => sum + (n.tx_bytes || 0), 0),
          };
        } catch {
          // Stats not available
        }
...
async function startContainer(id) {
  const client = getDockerClient();
  if (!client) throw new Error('Docker not available');
...
async function containerStats(id) {
  ...
  return {
    ...
    networkRx: Object.values(statsData.networks || {}).reduce((sum, n) => sum + (n.rx_bytes || 0), 0),
    networkTx: Object.values(statsData.networks || {}).reduce((sum, n) => sum + (n.tx_bytes || 0), 0),
  };
}
 ...
 */
async function composeDown(cwd) {
  const workDir = cwd || process.cwd();
 ...
 */
async function composeLogs(cwd, tail = 100) {
  const workDir = cwd || process.cwd();
...
let freeAgentStartTime = null;

/**
 * Start Free Agent sandbox container
 */
async function startFreeAgentSandbox(config) {
  const client = getDockerClient();
  if (!client) throw new Error('Docker not available');
 ...
 */
async function stopFreeAgentSandbox() {
  const client = getDockerClient();
  if (!client) throw new Error('Docker not available');
  ...
  try {
    ...
    throw new Error(`Failed to stop Free Agent: ${err.message}`);
  }
}
 ...
 */
async function getFreeAgentContainer() {
  const client = getDockerClient();
 ...
 */
async function execInFreeAgent(command, options = {}) {
  const client = getDockerClient();
  if (!client) throw new Error('Docker not available');
 ...
 */
async function restartFreeAgentSandbox() {
  const status = await getFreeAgentContainer();
 ...
 */
async function getFreeAgentLogs(tail = 100) {
  const status = await getFreeAgentContainer();
  if (!status.containerId) {
    return '';
  }
...
function registerDockerHandlers() {
  // Main docker handler
  ipcMain.handle('docker', async (_event, method, args = {}) => {
  ...
  ipcMain.handle('freeagent:start', async (_event, config) => {
  ...
  ipcMain.handle('freeagent:logs', async (_event, tail) => {
    return getFreeAgentLogs(tail);
  });
  
  ipcMain.handle('freeagent:exec', async (_event, args) => {
  ...
  console.log('[G-Rump] Docker IPC handlers registered');
}

/**
 * Unregister Docker IPC handlers
 */
function unregisterDockerHandlers() {
  ipcMain.removeHandler('docker');
  ipcMain.removeHandler('freeagent:start');
  ipcMain.removeHandler('freeagent:stop');
  ipcMain.removeHandler('freeagent:status');
  ipcMain.removeHandler('freeagent:restart');
  ipcMain.removeHandler('freeagent:logs');
  ipcMain.removeHandler('freeagent:exec');
}
